...
def do_clean(number=0):
    """ cleaning up the old archives"""
    number = int(number)
    if number <= 1:
        number = 1
    with lcd('versions'):
        archives = local('ls -t | grep web_static', capture=True).split('\n')
        delete_archive = archives[number:]
        for file in delete_archive:
            local('rm -f {}'.format(file))
    with cd('/data/web_static/releases'):
        archives = run('ls -t | grep web_static').split('\n')
        delete_archive = archives[number:]
        for file in delete_archive:
